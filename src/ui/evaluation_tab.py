...
class EvaluationTab(BaseTab):
    ...
    def setup_params_compare_ui(self):
        ...
        # 创建训练参数对比组件
        self.params_compare_component = ParamsCompareWidget(self)
        self.params_compare_component.set_main_window(self.main_window)
        
        # 连接信号
        self.params_compare_component.status_updated.connect(self.update_status)
        
        # 添加组件到布局
        params_compare_layout.addWidget(self.params_compare_component)
        
        # 如果已有模型目录，则设置给组件
        if self.model_dir:
            self.params_compare_component.set_model_dir(self.model_dir)
    ...
    def showEvent(self, event):
        ...
        if current_index == 6 and self.model_dir and hasattr(self, 'params_compare_component'):
            self.params_compare_component.load_model_configs()
    ...
    def apply_config(self, config):
        ...
        if 'default_param_save_dir' in config:
            ...
            if os.path.exists(param_dir):
                ...
                if hasattr(self, 'params_compare_component'):
                    self.params_compare_component.set_model_dir(param_dir)
    ...
    def browse_param_dir(self):
        ...
        if dir_path:
            ...
            # 更新组件的目录设置
            if hasattr(self, 'params_compare_component'):
                self.params_compare_component.set_model_dir(dir_path)
    ...
    def select_all_models(self):
        """选择所有模型"""
        if hasattr(self, 'params_compare_component'):
            self.params_compare_component.select_all_models()
            
    def deselect_all_models(self):
        """取消选择所有模型"""
        if hasattr(self, 'params_compare_component'):
            self.params_compare_component.deselect_all_models()
            
    def compare_params(self):
        """比较所选模型的训练参数"""
        if hasattr(self, 'params_compare_component'):
            self.params_compare_component.compare_params()
